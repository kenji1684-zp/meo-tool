...
type TabType = '月別集計' | '日別推移' | '曜日平均' | '使い方'
...
export default function PerformancePage() {
  ...
  // localStorage からキャッシュ読込
  useEffect(() => {
    const cache: MonthlyCache[] = []
    for (let i = 23; i >= 0; i--) {
      const ym  = format(subMonths(new Date(), i), 'yyyy/M')
      const raw = localStorage.getItem(`perf_month_${ym.replace('/', '-')}`)
      if (raw) {
  ...
  useEffect(() => {
    fetch('/api/business')
      .then(r => r.json())
      .then(d => { if (d.locations?.[0]) setLocationName(d.locations[0].name) })
      .catch(() => {})
  }, [])
  ...
  const fetchData = useCallback(async (ym: string) => {
    ...
    try {
      ...
      localStorage.setItem(`perf_month_${ym.replace('/', '-')}`, JSON.stringify(totals))
      ...
      try {
        ...
        localStorage.setItem(`perf_month_${prevYm.replace('/', '-')}`, JSON.stringify(prevTotals))
        setMonthlyCache(prev => {
  ...
  // 月別棒グラフ用データ（キャッシュから）
  const monthlyChartData = monthlyCache.map(c => ({
    month: (() => {
      const [, mo] = c.yearmonth.split('/').map(Number)
      return `${mo}月`
    })(),
  ...
  function exportKwCSV() {
    ...
    a.href = url
    a.download = `searched-keywords-${yearmonth.replace('/', '-')}.csv`
    a.click()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-gray-200 gap-0">
        {(['月別集計', '日別推移', '曜日平均', '使い方'] as TabType[]).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
